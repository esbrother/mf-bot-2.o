  ...
  if (interaction.commandName === 'play') {
    ...
    if (isUrl) {
      ...
      try {
        ...
        if (!info) {
          return interaction.editReply('❌ Enlace no soportado. Solo YouTube/SoundCloud.');
        }

        if (query.includes('youtu')) {
          songInfo = await play.video_info(query);
        } else {
          songInfo = { title: 'Canción de SoundCloud' };
        }

        const song = {
          title: songInfo.video_details?.title || 'Canción desde enlace',
          url: query,
          platform: query.includes('youtu') ? 'youtube' : 'soundcloud'
        };
